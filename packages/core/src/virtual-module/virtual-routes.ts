...
export function createVirtualRouteComponent(route: VirtualRoute): string {
  ...
  // Rename the component name to improve readability for error messages.
  const componentName = "Astrobook_Component_" + slug(route.storyModule.id)
...
const ${componentName}= mod.default.component
---
...
<StoryPage story={'${route.props.story}'} hasSidebar={${route.props.hasSidebar}}>
  <WithDecorators decorators={decorators}>
    {isAstroComponent ? (
      ...
      <${componentName} {...args} client:load />
    )}
  </WithDecorators>
</StoryPage>
`.trim()
